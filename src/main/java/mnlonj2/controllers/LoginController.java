...
public class LoginController {
   ...
   * @param loginHandler A collaborator which helps to build HTTP response
  ...
  /**
   * @param usernamePasswordCredentials An instance of
   * {@link UsernamePasswordCredentials} in the body payload
   * @param request The {@link HttpRequest} being executed
   * @return An AccessRefreshToken encapsulated in the HttpResponse or a failure
   * indicated by the HTTP status
   */
  ...
  public Single<HttpResponse> login(@Valid @Body UsernamePasswordCredentials usernamePasswordCredentials, HttpRequest<?> request) {
    Class<? extends HttpRequest> aClass = request.getClass();
    InetSocketAddress remoteAddress = request.getRemoteAddress();
    remoteAddress.getHostString();
    
    ml("EN MI LOGIN"+remoteAddress.getHostString()+"\n "+aClass);
  ...
  private void ml(String s) {
    System.out.println(Cte.ANSI_BRIGHT_BG_BLUE + " " + Cte.ANSI_RED + " " + s + " " + Cte.ANSI_RESET);
